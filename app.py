...
DISEASE_INFO = {
    "Blight": {
        ...
        "description": (
            "Large irregular lesions with water-soaked margins that turn brown. "
            "Caused by Exserohilum turcicum or Bipolaris maydis. "
            "Can destroy entire canopies if left untreated."
        ),
        ...
        "prevention": (
            "• Plant blight-resistant hybrid varieties.\n"
    ...
    "Common Rust": {
        ...
        "description": (
            "Orange-brown powdery pustules appear on both leaf surfaces. "
            "Caused by Puccinia sorghi. Spreads rapidly through wind-borne spores "
            "in cool, humid conditions."
        ),
        "cause": "Fungal — Puccinia sorghi. Wind-borne spores.",
        "treatment": (
            "• Apply mancozeb (2g/L) or propiconazole (1mL/L).\n"
    ...
    "Gray Leaf Spot": {
        ...
        "description": (
            "Long, narrow rectangular lesions running parallel to leaf veins. "
            "Caused by Cercospora zeae-maydis. Considered the most yield-limiting "
            "foliar disease of maize globally."
        ),
    ...
    "Healthy": {
        ...
        "description": (
            "No disease detected. Leaves show uniform green colour with no lesion, "
            "pustule or spot features. Plant is growing normally."
        ),
...
<style>
  ...
  .sb-card {
    background: #151515; border-left: 3px solid #ff6d00;
  ...
  .hero {
    background: linear-gradient(135deg, #0f0f0f 0%, #1a0a00 60%, #0f0f0f 100%);
    border: 1px solid #2a1500; border-radius: 20px;
    padding: 2rem 2.5rem; margin-bottom: 1.5rem;
  }
  .hero-tag {
    display: inline-block; background: #ff6d0022;
    border: 1px solid #ff6d0055; color: #ff6d00 !important;
  ...
  .hero h1 {
    ...
    margin: 0 0 0.4rem; line-height: 1.2; color: #ffffff !important;
  }
  .hero h1 span {
    background: linear-gradient(90deg, #ff6d00, #ffab40);
    -webkit-background-clip: text; -webkit-text-fill-color: transparent;
  ...
  .input-card h4 {
    ...
    color: #ff6d00 !important; margin: 0 0 0.2rem;
  }
  .input-card small { color: #444 !important; font-size: 0.76rem; }

  .result-card {
    background: #111; border-radius: 14px;
    padding: 1.2rem 1.5rem; margin-top: 0.5rem; border: 1px solid #1e1e1e;
  }
  ...
  .info-block {
    ...
    border-left: 3px solid #ff6d00; border-radius: 8px;
    padding: 0.8rem 1rem; font-size: 0.85rem; color: #aaa !important;
    line-height: 1.8; white-space: pre-line; margin-top: 0.4rem;
  }

  .stTabs [data-baseweb="tab-list"] {
    background: #0f0f0f; border-radius: 10px; padding: 4px;
    gap: 6px; border: 1px solid #1a1a1a; margin-bottom: 1rem;
  }
  .stTabs [data-baseweb="tab"] {
    border-radius: 8px; color: #555 !important;
  ...
  .warn-box {
    background: #1a1000; border: 1px solid #ff6d0044;
    border-radius: 8px; padding: 0.6rem 0.9rem;
    font-size: 0.82rem; color: #ffab40 !important; margin-top: 0.6rem;
  }

  .guide-card {
    background: #111; border-radius: 12px;
    border: 1px solid #1a1a1a; padding: 1rem; text-align: center;
  }

  .footer {
    text-align: center; font-size: 0.73rem; color: #2a2a2a !important;
...
with st.sidebar:
    ...
    st.markdown(
        "<small style='color:#444'>Crop Disease Detection System</small>",
    ...
    for k, v in [
        ...
        st.markdown(
            f'<div class="sb-card"><b>{k}:</b> {v}</div>',
            unsafe_allow_html=True,
        )
...
with tab1:
    ...
    if image_source is not None:
        ...
        with col_img:
            ...
            st.markdown(
                f"<small style='color:#333'>"
                f"Size: {pil_image.width} × {pil_image.height} px"
                f"</small>",
                unsafe_allow_html=True,
            )
            ...
            if model is None:
                ...
                for i, cls in enumerate(CLASS_NAMES):
                    ...
                    c1.markdown(
                        ...
                        f"{DISEASE_INFO[disp]['emoji']} {disp}"
                        f"</span>",
                        unsafe_allow_html=True,
                    )
        ...
        for col, (cls, inf) in zip(cols, DISEASE_INFO.items()):
            with col:
                st.markdown(
                    ...
                    f'<small style="color:{inf["severity_color"]}">'
                    f'{inf["severity"]} severity</small>'
                    f'<br><br>'
                    f'<small style="color:#444;font-size:0.73rem">'
                    f'{inf["description"][:65]}…</small>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
...
with tab2:
    ...
    st.markdown(
        "Detailed information on each disease class this model was trained to identify."
    )
    st.markdown("---")
    for cls, inf in DISEASE_INFO.items():
...
EfficientNetB0 base weights were frozen. Only the classification head
(GlobalAveragePooling → Dense 128 → Dense 4) was trained for up to 25 epochs
with a learning rate of 3×10⁻⁴. Early stopping prevented overfitting.

**Phase 2 — Fine-Tuning**
The top 100 layers of EfficientNetB0 were unfrozen and trained at a much lower
learning rate of 5×10⁻⁶ for up to 10 epochs. This allowed the model to adapt
pre-learned ImageNet features to maize disease patterns.
...
st.markdown(
    ...
    'EfficientNetB0 · Streamlit · Transfer Learning'
    '</div>',
    unsafe_allow_html=True,
)
